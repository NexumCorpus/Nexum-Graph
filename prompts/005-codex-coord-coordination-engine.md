# Prompt 005 — codex-coord Coordination Engine

## Context

You are implementing the **Phase 2 coordination engine** for Project Codex.
...
The coordination engine builds on `CodeGraph` from codex-graph for its
...
### `crates/codex-coord/src/coordinator.rs`
...
These types are in `codex-core/src/coordination.rs`:
...
Also in `codex-core/src/error.rs`:
```rust
pub enum CodexError {
    // ... existing variants ...
...
All 16 tests in `crates/codex-coord/tests/coordination.rs` must pass:
...
cargo clippy -p codex-coord -- -D warnings    # No warnings
cargo fmt -p codex-coord --check              # Formatted
```
...
3. Do NOT modify `codex-coord/src/detector.rs` — Phase 1 is done
4. Do NOT modify test files — tests are acceptance criteria
5. Do NOT add new dependencies to any Cargo.toml
6. The ONLY file you modify is `crates/codex-coord/src/coordinator.rs`
...
| File | Action |
|------|--------|
| `crates/codex-coord/src/coordinator.rs` | Fill 8 `todo!()` bodies |
