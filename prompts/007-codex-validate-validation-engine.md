...
You are implementing the **Phase 2 pre-commit validation engine** for Project Codex.
...
This prompt adds:
1. **Validation engine** — `ValidationEngine::validate()` in `codex-validate`
2. **CLI pipeline** — `run_validate` in `codex-cli/coordination_pipeline.rs`
3. **Output formatting** — `format_validation_report` in `codex-cli/output.rs`
4. **Integration tests** — `codex-cli/tests/validation_cli.rs`
...
### 1. `crates/codex-validate/src/validator.rs` — 7 functions
...
### 2. `crates/codex-cli/src/coordination_pipeline.rs` — 1 function
...
```rust
use codex_core::CodexError;       // already imported
use codex_parse::SemanticExtractor; // already available via codex_parse
```

### 3. `crates/codex-cli/src/output.rs` — 2 functions
...
```rust
fn validation_kind_label(kind: &codex_core::ValidationKind) -> &'static str {
    match kind {
        codex_core::ValidationKind::UnlockedModification { .. } => "UnlockedModification",
        codex_core::ValidationKind::UnlockedDeletion { .. } => "UnlockedDeletion",
        codex_core::ValidationKind::BrokenReference { .. } => "BrokenReference",
        codex_core::ValidationKind::StaleCallers { .. } => "StaleCallers",
    }
}
```
...
No new imports needed. `codex_core::CodexError`, `codex_core::SemanticLock`,
and `codex_parse::SemanticExtractor` are already accessible. Add `codex_validate`
usage inline (it's already in `Cargo.toml` dependencies).

### In `output.rs` (already present)
```rust
use codex_core::ValidationReport; // already imported
```

## Type Definitions (FROZEN — Do NOT Modify)

All types from `codex-core` are frozen. The relevant ones for this prompt:

```rust
// codex-core/src/validation.rs
...
Create `crates/codex-cli/tests/validation_cli.rs` with the following tests.
...
//! - ValidationEngine (codex-validate) checks lock coverage and reference integrity
...
use codex_cli::coordination_pipeline::{agent_name_to_id, run_lock, run_validate};
use codex_cli::output::format_validation_report;
use std::path::Path;
...
fn validate_clean_with_delete_lock() {
    ...
    use codex_cli::coordination_pipeline::{save_locks, LockEntry};
    use codex_core::IntentKind;
    ...
    let files = codex_cli::pipeline::collect_files_at_ref(&git_repo, "HEAD~1", &extractors).unwrap();
    let graph = codex_cli::pipeline::build_graph(&files, &extractors).unwrap();
...
fn validate_detects_unlocked_modification() {
    ...
    assert!(
        issue.suggestion.as_ref().unwrap().contains("codex lock"),
        "suggestion should mention codex lock command"
    );
}
...
fn validate_unlocked_modification_is_error_severity() {
    ...
    let unlocked_mod = report.issues.iter()
        .find(|i| matches!(i.kind, codex_core::ValidationKind::UnlockedModification { .. }));
    assert!(unlocked_mod.is_some(), "should have UnlockedModification issue");
    assert_eq!(unlocked_mod.unwrap().severity, codex_core::Severity::Error);
}
...
fn validate_detects_unlocked_deletion() {
    ...
    let deletion_issue = report.issues.iter()
        .find(|i| matches!(i.kind, codex_core::ValidationKind::UnlockedDeletion { .. }));
...
fn validate_detects_stale_caller_after_signature_change() {
    ...
    let stale = report.issues.iter()
        .find(|i| matches!(i.kind, codex_core::ValidationKind::StaleCallers { .. }));
    assert!(stale.is_some(), "should detect stale caller");
    assert_eq!(stale.unwrap().severity, codex_core::Severity::Warning);
    assert!(
...
fn validate_detects_broken_reference() {
    ...
    let broken = report.issues.iter()
        .find(|i| matches!(i.kind, codex_core::ValidationKind::BrokenReference { .. }));
    assert!(broken.is_some(), "should detect broken reference");
    assert_eq!(broken.unwrap().severity, codex_core::Severity::Error);
    assert!(
...
```
cargo test -p codex-parse         # 16 tests
cargo test -p codex-graph         # 20 tests
cargo test -p codex-coord         # 28 tests
cargo test -p codex-cli           # 29 existing + 15 new validation tests
```

New validation tests in `codex-cli`:
```
...
Additionally:
```
cargo clippy -p codex-validate -p codex-cli -- -D warnings    # No warnings
cargo fmt -p codex-validate -p codex-cli --check              # Formatted
```

## Constraints

1. Do NOT modify any file in `codex-core/` — types are frozen
2. Do NOT modify `codex-validate/src/lib.rs`
3. Do NOT modify `codex-coord/` — Phase 1 + 2 engine are complete
4. Do NOT modify `codex-cli/src/cli.rs`, `codex-cli/src/main.rs`, or `codex-cli/src/lib.rs`
5. Do NOT modify `codex-cli/src/pipeline.rs`
...
9. The ONLY files you modify are:
   - `crates/codex-validate/src/validator.rs` (fill 7 `todo!()` bodies)
   - `crates/codex-cli/src/coordination_pipeline.rs` (fill 1 `todo!()` body: `run_validate`)
   - `crates/codex-cli/src/output.rs` (fill 2 `todo!()` bodies)
10. The ONLY file you create is:
    - `crates/codex-cli/tests/validation_cli.rs`
...
| `crates/codex-validate/src/validator.rs` | Fill 7 `todo!()` bodies |
| `crates/codex-cli/src/coordination_pipeline.rs` | Fill 1 `todo!()` body (`run_validate`) |
| `crates/codex-cli/src/output.rs` | Fill 2 `todo!()` bodies (`format_validation_report`, `validation_kind_label`) |
| `crates/codex-cli/tests/validation_cli.rs` | CREATE — 15 integration tests |
