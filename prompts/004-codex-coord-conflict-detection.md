# Prompt 004 — codex-coord Conflict Detection Implementation

## Context

You are implementing the **Phase 1 conflict detection engine** for Project Codex.
...
Phase 0 (`codex diff`) is already complete and working. You will reuse its
patterns (git2 tree walking, codex-parse extraction, codex-graph construction).
...
### 1. `crates/codex-coord/src/detector.rs`
...
3. Create extractors: `vec![codex_parse::typescript_extractor()]`
...
This is nearly identical to `codex-cli/src/pipeline.rs` functions. Reuse the
same pattern:
...
**Reference implementation** (from `crates/codex-cli/src/pipeline.rs`):
```rust
...
`codex-cli` — the coordination crate should not depend on the CLI crate.
...
### 2. `crates/codex-cli/src/output.rs`
...
These types are in `codex-core/src/conflict.rs` and must not be changed:
...
`codex-coord/Cargo.toml`:
```toml
codex-core = { workspace = true }
codex-parse = { workspace = true }
codex-graph = { workspace = true }
git2 = { workspace = true }
```

`codex-cli/Cargo.toml`:
```toml
codex-coord = { workspace = true }
```
...
In `output.rs` (already has `use codex_core::{ChangeKind, ConflictReport, SemanticDiff};`):
```rust
// You may need to add:
use codex_core::Severity;
```

## Acceptance Criteria

All 9 tests in `crates/codex-coord/tests/conflict_detection.rs` must pass:

```
cargo test -p codex-coord
```
...
Additionally:
```
cargo test -p codex-cli          # Phase 0 tests must still pass (17/17)
cargo clippy -p codex-coord -p codex-cli -- -D warnings  # No warnings
cargo fmt -p codex-coord -p codex-cli --check             # Formatted
```

## Constraints

1. Do NOT modify any file in `codex-core/` — types are frozen
2. Do NOT modify `codex-coord/src/lib.rs` — module structure is set
3. Do NOT modify test files — tests are acceptance criteria
4. Do NOT modify `codex-cli/src/cli.rs` — CLI args are frozen
5. Do NOT import from `codex-cli` in `codex-coord` (no circular dependency)
...
| File | Action |
|------|--------|
| `crates/codex-coord/src/detector.rs` | Fill 4 `todo!()` bodies |
| `crates/codex-cli/src/output.rs` | Fill 1 `todo!()` body (`format_report`) |
