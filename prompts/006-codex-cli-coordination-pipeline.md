...
You are implementing the **Phase 2 CLI coordination pipeline** for Project Codex.
...
2. **CLI pipeline** — `codex lock`, `codex unlock`, `codex locks` commands
...
### 1. `crates/codex-coord/src/coordinator.rs` — 2 new methods
...
### 2. `crates/codex-cli/src/coordination_pipeline.rs` — 8 functions
...
Read `.codex/locks.json`. Return empty vec if file doesn't exist:
```rust
...
Write `.codex/locks.json`, creating the directory if needed:
```rust
...
### 3. `crates/codex-cli/src/output.rs` — 2 new functions
...
```rust
use codex_core::{AgentId, CodexError, CodexResult, IntentKind, LockResult, SemanticId, SemanticUnit};
...
All types from `codex-core` (`AgentId`, `Intent`, `IntentKind`,
`SemanticLock`, `LockResult`, `LockConflict`, `CoordinationState`,
`CodexError`, `SemanticUnit`, `SemanticId`) are frozen.

## Acceptance Criteria

All tests in `codex-coord` must pass (28 total: 9 Phase 1 + 16 Phase 2 + 3 new):
```
...
All tests in `codex-cli` must pass (28 total: 17 Phase 0 + 11 new):
```
...
2. Do NOT modify `codex-coord/src/lib.rs` or `codex-coord/src/detector.rs`
3. Do NOT modify `codex-cli/src/cli.rs`, `codex-cli/src/main.rs`, or `codex-cli/src/lib.rs`
4. Do NOT modify `codex-cli/src/pipeline.rs`
...
7. The ONLY files you modify are:
   - `crates/codex-coord/src/coordinator.rs` (add 2 methods)
   - `crates/codex-cli/src/coordination_pipeline.rs` (fill 8 `todo!()` bodies)
...
| File | Action |
|------|--------|
| `crates/codex-coord/src/coordinator.rs` | Fill 2 `todo!()` bodies (`export_locks`, `import_locks`) |
| `crates/codex-cli/src/coordination_pipeline.rs` | Fill 8 `todo!()` bodies |
| `crates/codex-cli/src/output.rs` | Fill 2 `todo!()` bodies (`format_lock_result`, `format_locks`) |
