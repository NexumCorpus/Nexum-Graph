# Prompt 003 — codex-cli: `codex diff` Pipeline Implementation

## Role

You are implementing the `codex diff` CLI pipeline for Project Codex. This is the
integration crate that wires together `codex-parse` (extraction), `codex-graph`
...
1. `crates/codex-cli/src/pipeline.rs` — git file collection, graph building, diff orchestration
2. `crates/codex-cli/src/output.rs` — JSON, text, and GitHub markdown formatters

Do NOT modify:
- `crates/codex-cli/src/cli.rs` (clap definitions — complete)
- `crates/codex-cli/src/lib.rs` (module re-exports — complete)
- `crates/codex-cli/src/main.rs` (binary entry point — complete)
- `crates/codex-core/src/` (authoritative types — FROZEN)
- `crates/codex-parse/src/` (extractor implementation — FROZEN)
- `crates/codex-graph/src/` (graph implementation — FROZEN)
- `crates/codex-cli/tests/diff_pipeline.rs` (test file — FROZEN)
...
2. Create the extractor list: `vec![codex_parse::typescript_extractor()]`
...
The types you'll work with (from `codex-core`, do NOT modify):
...
## CodeGraph API (from `codex-graph`, already implemented):
...
## SemanticExtractor trait (from `codex-parse`):
...
Run with: `cargo test -p codex-cli`
...
1. `cargo test -p codex-cli` — all 17 tests pass
2. `cargo clippy -p codex-cli -- -D warnings` — clean
3. `cargo fmt -p codex-cli --check` — clean
