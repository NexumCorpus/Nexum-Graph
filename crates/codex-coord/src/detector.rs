...
///
/// Reuses the same collect_files + parse + graph pipeline from codex-cli.
...
fn push_signature_mismatch_conflicts(
    conflicts: &mut Vec<SemanticConflict>,
    modified_units: &[codex_core::ModifiedUnit],
    other_graph: &CodeGraph,
    changed_branch: &str,
    caller_branch: &str,
) {
...
fn push_concurrent_modification_conflicts(
    ...
    let modified_a: HashMap<&str, &codex_core::ModifiedUnit> = diff_a
        .modified
        .iter()
        .map(|modified| (modified.after.qualified_name.as_str(), modified))
        .collect();
    let modified_b: HashMap<&str, &codex_core::ModifiedUnit> = diff_b
        .modified
        .iter()
        .map(|modified| (modified.after.qualified_name.as_str(), modified))
        .collect();
