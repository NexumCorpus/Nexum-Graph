//! Integration tests for the codex-coord conflict detection engine.
//!
...
//! git2 → codex-parse → codex-graph → SemanticDiff → cross-reference → ConflictReport

use codex_coord::ConflictDetector;
use codex_core::{ConflictKind, Severity};
use std::path::Path;
...
fn conflict_report_counts_and_exit_codes() {
    use codex_core::{ConflictReport, SemanticConflict, SemanticUnit, UnitKind};
