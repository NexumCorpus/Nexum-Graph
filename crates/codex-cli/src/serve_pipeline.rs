//! Async coordination server pipeline for `codex serve`.
...
use chrono::Utc;
use codex_coord::{CoordEvent, CoordinationService, GraphQuery, IntentPayload, IntentResult};
use codex_core::{CodexError, CodexResult};
use codex_eventlog::{EventLog, Mutation, SemanticEvent};
...
pub async fn run_serve(repo_path: &Path, host: &str, port: u16) -> CodexResult<()> {
    ...
    println!("codex serve listening on http://{}", handle.local_addr());
...
pub async fn spawn_server(repo_path: &Path, bind_addr: SocketAddr) -> CodexResult<ServerHandle> {
    ...
    let state = AppState {
        ...
        events,
        event_log_path: repo_path.join(".codex").join("events.json"),
    };
...
) -> Result<Json<Vec<codex_core::SemanticUnit>>, ApiError> {
    let service = state.service.lock().await;
    Ok(Json(service.query_graph(&query)?))
}

async fn list_locks(State(state): State<AppState>) -> Json<Vec<codex_coord::LockEntry>> {
    let service = state.service.lock().await;
    Json(service.locks())
}
