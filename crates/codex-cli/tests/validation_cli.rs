...
fn validate_clean_with_delete_lock() {
    ...
    use codex_cli::coordination_pipeline::{LockEntry, save_locks};
    ...
    let files =
        codex_cli::pipeline::collect_files_at_ref(&git_repo, "HEAD~1", &extractors).unwrap();
...
fn validate_unlocked_modification_is_error_severity() {
    ...
    let unlocked_mod = report.issues.iter().find(|issue| {
        matches!(
            issue.kind,
            codex_core::ValidationKind::UnlockedModification { .. }
        )
    });
    assert!(
...
fn validate_detects_unlocked_deletion() {
    ...
    let deletion_issue = report.issues.iter().find(|issue| {
        matches!(
            issue.kind,
            codex_core::ValidationKind::UnlockedDeletion { .. }
        )
    });
...
fn validate_detects_stale_caller_after_signature_change() {
    ...
    let stale = report
        .issues
        .iter()
        .find(|issue| matches!(issue.kind, codex_core::ValidationKind::StaleCallers { .. }));
...
fn validate_detects_broken_reference() {
    ...
    let broken = report.issues.iter().find(|issue| {
        matches!(
            issue.kind,
            codex_core::ValidationKind::BrokenReference { .. }
        )
    });
