...
use codex_cli::eventlog_pipeline::{run_log, run_replay, run_rollback};
use codex_cli::output::{format_event_log, format_replay_state, format_rollback_outcome};
use codex_core::{SemanticUnit, UnitKind};
...
fn repo_log(repo: &std::path::Path) -> EventLog {
    EventLog::new(repo.join(".codex").join("events.json"))
}
...
fn format_rollback_outcome_text_variants() {
    ...
    let blocked = format_rollback_outcome(
        &codex_eventlog::RollbackOutcome {
            original_intent_id: Uuid::new_v4(),
            rollback_event: None,
            conflicts: vec![codex_eventlog::RollbackConflict {
                unit: feature.id,
