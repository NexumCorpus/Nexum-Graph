use chrono::Utc;
use codex_cli::serve_pipeline::{AbortRequest, CommitRequest, spawn_server};
use codex_coord::{CoordEvent, IntentPayload, IntentResult, LockEntry, PlannedChange};
use codex_core::SemanticUnit;
use codex_eventlog::{Mutation, SemanticEvent};
...
async fn serve_exposes_declare_locks_commit_and_abort() {
    ...
    after.body_hash += 1;
    let commit_response: codex_cli::serve_pipeline::CommitResponse = client
    ...
    let events_path = repo_path.join(".codex").join("events.json");
    ...
    let abort_response: codex_cli::serve_pipeline::AbortResponse = client
