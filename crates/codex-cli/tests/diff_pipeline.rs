//! Integration tests for the `codex diff` pipeline.
//!
...
//! git2 → codex-parse → codex-graph → SemanticDiff → output
...
fn build_graph_single_function() {
    ...
    let extractors: Vec<Box<dyn codex_parse::SemanticExtractor>> = vec![typescript_extractor()];
...
fn format_json_produces_valid_json() {
    let diff = codex_core::SemanticDiff {
        added: vec![],
        removed: vec![],
        modified: vec![],
        moved: vec![],
